...
package org.apache.nutch.urlfilter.baidu;
 ...
 * 
 */
public class BaiduURLFilter implements URLFilter {

	private static final Logger LOG = LoggerFactory.getLogger(BaiduURLFilter.class);

	private List<String> forbids;
	private Configuration conf;
	private String attributeFile;
	
	public BaiduURLFilter() throws IOException {

	}

	public BaiduURLFilter(Reader reader) throws IOException {
	...
	public void readConfiguration(Reader reader) throws IOException {
		...
		if (reader == null) {
			if (LOG.isWarnEnabled()) {
				LOG.warn("Missing urlfilter.baidu.file, all URLs will be rejected!");
			}
			return;
		}
	...
	public static void main(String args[]) throws IOException {
		...
		BaiduURLFilter filter;
		if (args.length >= 1)
			filter = new BaiduURLFilter(new FileReader(args[0]));
		else {
			filter = new BaiduURLFilter();
	...
	public void setConf(Configuration conf) {
		...
		String pluginName = "urlfilter-baidu";
		...
		String file = conf.get("urlfilter.baidu.file");
